// access context menu API
jetpack.future.import("menu");
jetpack.future.import("clipboard");

/*
I love me some pageMods, but right now there are multiple-invocations issues

jetpack.future.import("pageMods");

jetpack.pageMods.add(function(d) {
*/
jetpack.tabs.onReady(function(d) {
    ...
    jetpack.menu.beforeShow = function (menu, context) {
        var style = context.document.getElementById("bztw_cc");
        menu.reset();
        menu.add({
            label: (style.disabled ? "Hide CC History" : "Show CC History"),
            command: function() style.disabled = !style.disabled
        });
        if (CheckinComment.isValid()) {
            var message = context.document.getElementById("__bz_tw_checkin_comment");
            if (message) {
                message = message.textContent;
                menu.add({
                    label: "Copy Check-in Comment",
                    command: function() jetpack.clipboard.set(message)
                });
            }
        }
    };
