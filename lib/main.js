...
function tweakBugzilla(d) {
    ...
    function getCCMenuItemLabel(d) {
        var style = d.getElementById("bztw_cc");
        return (style.disabled ? "Hide CC History" : "Show CC History");
    }
    var ccMenuItem = contextMenu.Item({
        label: getCCMenuItemLabel(d),
        onClick: function(context, item) {
            style.disabled = !style.disabled;
            item.label = getCCMenuItemLabel(context.document);
        }
    });
    ...
    if (CheckinComment.isValid()) {
        ...
        if (message) {
            ...
            var copyCheckinCommentItem = contextMenu.Item({
                ...
                onClick: function() jetpack.clipboard.set(message)
            });
